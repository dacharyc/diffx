...
func TestShiftBoundaries_BlankLines(t *testing.T) {
	...
	// The blank lines should ideally not be part of delete/insert operations
	for _, op := range ops {
		if op.Type == Delete || op.Type == Insert {
			// Check that blank lines at boundaries are not included in changes
			if op.Type == Delete && op.AEnd > op.AStart {
				for i := op.AStart; i < op.AEnd; i++ {
					if a[i] == "" {
						// This is acceptable if the blank line is in the middle of changes
						// We're mainly checking that the algorithm runs correctly
					}
				}
			}
		}
